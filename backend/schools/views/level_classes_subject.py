...
from backend.schools.models import LevelClasses,Stream, School, ProgramLevelTemplate, SubjectRepository, GradingScale, SchoolSubject
from backend.schools.forms import LevelClassesForm, ClassWithoutProgramLevelFieldForm, NoStreamForm, SubjectRepositoryForm, SchoolSubjectForm
...
from backend.schools.models import Staff  # Assuming staff is in a separate app

def class_detail_view(request, pk):
    # Get the class object
    class_obj = get_object_or_404(LevelClasses, pk=pk)

    # Enrolled students
    enrolled_students = class_obj.enrolled_students.select_related('student')

    # Subjects
    subjects = SubjectRepository.objects.filter(
        program_levels=class_obj.program_level_template
    ).distinct()
    ...
    return render(request, 'schools/class_details.html', context)
