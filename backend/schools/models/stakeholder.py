...
class Staff(models.Model):
    ...
    profile_picture = models.ImageField(upload_to='staff_profiles/%Y/%m/%d/', blank=True, null=True,)

    def save(self, *args, **kwargs):
        if self.pk and self.profile_picture:
            try:
                old_profile_picture = Staff.objects.get(pk=self.pk).profile_picture
            ...
            if old_profile_picture and self.profile_picture != old_profile_picture:
                ...
                if not remove_old_file(old_profile_picture.path):
                    # Handle the case where file removal failed
                    import logging
                    logger.warning("Failed to remove old profile picture file.")  
